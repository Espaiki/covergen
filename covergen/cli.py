...
_CONTENT_KEYS = (
    "institution_name", "subject_name", "teacher_name",
    "student_name", "grade_course", "school_year",
)
...
def _resolve_inputs(
    ...
    if config_path is not None:
        ...
        bg_path_str = data.pop("background", "")
...
def generate(
    ...
    format_profile: FormatProfile = typer.Option(
        FormatProfile.SPIRAL_NOTEBOOK, "--format", "-f", case_sensitive=False,
    ),
    background: Optional[Path] = typer.Option(
        None, "--background", "-b",
        help="Ruta a PNG/JPG para el fondo de la carátula.",
    ),
    theme: Theme = typer.Option(
        Theme.MINIMAL_GEOMETRIC, "--theme", "-t",
        case_sensitive=False,
        help="Estilo visual: minimal-geometric | classic-academic.",
    ),
    decorations: int = typer.Option(
        0, "--decorations", "-d", min=0, max=4,
        help="Número de ornamentos SVG en esquinas (0–4).",
    ),
    auto_contrast: bool = typer.Option(
        True, "--auto-contrast/--no-auto-contrast",
        help="Ajusta la paleta al brillo del fondo automáticamente.",
    ),
    ...
    # Resolve palette (auto-contrast or default light_text)
    palette_obj = None
    palette_mode = "light_text"
    if bg_b64 and auto_contrast:
        from .contrast import auto_select_palette
        palette_obj  = auto_select_palette(base64.b64decode(bg_b64), theme)
        palette_mode = "auto"
    else:
        palette_obj = get_palette(theme, "light_text")

    # Resolve decorations
    deco_slots = pick_decorations(theme, count=decorations, seed=42) if decorations else None
    ...
    bg_label = "imagen personalizada" if bg_b64 else "gradiente CSS (sin imagen)"
    ...
    summary.add_row("[bold]Tema[/]",        theme.value)
    summary.add_row("[bold]Paleta[/]",      f"modo={palette_mode}")
    summary.add_row("[bold]Decoraciones[/]", str(decorations))
...
def formats_list() -> None:
    ...
    table.add_column("Margen seguro", justify="right")
...
def backgrounds_list() -> None:
    ...
    imgs = [p for p in sorted(assets_dir.iterdir())
    ...
    for p in imgs:
        table.add_row(p.name, f"{p.stat().st_size / 1024:.1f} KB")
    console.print(table)
